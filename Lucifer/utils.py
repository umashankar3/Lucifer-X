...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        sys.modules["Lucifer.utils"] = Lucifer.utils
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[Here](https://t.me/Lucifer_support_group)"
            ...
            ftext += "--------BEGIN Lucifer USERBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Lucifer/plugins/assistant/{shortname}.py")
        name = "Lucifer.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        sys.modules[
            "Lucifer.plugins.assistant" + "Initialising Lightning" + shortname
        ] = mod
        sedprint.info("Lucifer Has imported " + shortname)
...
def load_assistant(shortname):
    if shortname.startswith("__"):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        print("Initialising Lucifer.")
        print("Lucifer - Imported " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["Lucifer.plugins.assistant." + shortname] = mod
        print("Lucifer Has imported " + shortname)
